...
ggsave(plot = p, filename = "steak_R.png", path = "figures/explanatory_figures", width = 8, height = 4)
```
...
library(tidyverse)
cheat <- steak %>%
  select(Gender, didcheat = Have.you.ever.cheated.on.your.significant.other.)%>%
  filter(Gender!="", didcheat!="")%>%
  group_by(Gender, didcheat)%>%
  summarize(no=n())%>%
  mutate(no=as.numeric(no))%>%
  unite(gendercheat, c(Gender, didcheat), sep = " ")

b <- ggplot(data = cheat, (aes(x = gendercheat, y = no, fill = gendercheat))) + 
  geom_bar(stat = 'identity') +
  labs(title="Amount of cheaters in survey.") +
  xlab("")+
  ylab("")+
  theme_bw()
...
## save plot to figures/explanatory_figures directory
ggsave(plot = b, filename = "meat-plotR.png", path = "figures/explanatory_figures", width = 8, height = 4)
```
